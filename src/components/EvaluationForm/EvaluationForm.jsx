...
import { Container, Button, TextField, MenuItem, Typography, Checkbox, ListItemText, Select, RadioGroup, FormControlLabel, Radio, Grid, Box, Modal, Alert } from '@mui/material'; 
...
const EvaluationForm = () => {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      ...
      console.log('Valores del formulario:', values); // Aquí se muestra el console.log con los valores del formulario
      resetForm();
      setOpen(true);
    } catch (error) {
            ...
            <Form>
                <Grid container spacing={2}>
                    <Grid item xs={6}>
                        ...
                        <Field name={initialQuestions[0].id.toString()} as={TextField} select fullWidth error={touched[initialQuestions[0].id.toString()] && !!errors[initialQuestions[0].id.toString()]} helperText={touched[initialQuestions[0].id.toString()] && errors[initialQuestions[0].id.toString()]}>
                ...
                <div key={question.id} style={{ marginBottom: '16px', marginTop: index === 0 ? '21px' : '0' }}>
                    ...
                    <Field name={question.id.toString()} as={TextField} select fullWidth error={touched[question.id.toString()] && !!errors[question.id.toString()]} helperText={touched[question.id.toString()] && errors[question.id.toString()]}>
